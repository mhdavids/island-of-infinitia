...
(function() {
    ...
    const Puzzles = (function() {
        const puzzles = [
            ...
            { topic: '1.2', type: 'multiple_choice', stem: 'If $\\lim_{x \\to a} f(x) = L$, when does $f(a) = L$?', choices: ['Whenever $f(a)$ is defined', 'Whenever $L$ is finite', 'Whenever $f$ is continuous at $a$', 'Whenever $L \\neq 0$'], correct: 2, explanation: 'By definition, $f$ is continuous at $a$ when $\\lim_{x \\to a} f(x) = f(a)$. The limit equaling $f(a)$ is exactly continuity.' },
            ...
            { topic: '1.4', type: 'multiple_choice', stem: 'A limitation of tables for limits:', choices: ['Can only be used for one-sided limits', 'Give estimates, not proofs', 'Only work for polynomials', 'Can\'t detect infinite limits'], correct: 1, explanation: 'Tables show patterns but can\'t prove limits definitively.' },
            ...
            { topic: '1.5', type: 'multiple_choice', stem: 'If $\\lim f = 6$ and $\\lim g = 0$, then $\\lim \\frac{f}{g}$:', choices: ['Equals $0$', 'Equals $6$', 'Must equal $\\infty$', 'Cannot be determined without more information'], correct: 3, explanation: 'When the denominator approaches 0 and the numerator does not, the limit could be $+\\infty$, $-\\infty$, or DNE depending on how $g$ approaches 0.' },
            ...
            // Topic 1.7
            { topic: '1.7', type: 'multiple_choice', stem: '$\\lim_{x \\to 5} \\frac{x^2 - 25}{x - 5} =$', choices: ['$0$', '$5$', '$10$', '$25$'], correct: 2, explanation: 'Factor: $\\frac{(x-5)(x+5)}{x-5} = x + 5 \\to 10$ as $x \\to 5$.' },
            { topic: '1.7', type: 'multiple_choice', stem: '$\\lim_{x \\to 9} \\frac{\\sqrt{x} - 3}{x - 9} =$', choices: ['$0$', '$\\frac{1}{6}$', '$\\frac{1}{3}$', '$6$'], correct: 1, explanation: 'Since $x - 9 = (\\sqrt{x}-3)(\\sqrt{x}+3)$, we get $\\frac{1}{\\sqrt{x}+3} \\to \\frac{1}{6}$.' },
            ...
            { topic: '1.7', type: 'multiple_choice', stem: '$\\lim_{x \\to 0} \\frac{\\sin(5x)}{x} =$', choices: ['$0$', '$1$', '$5$', 'DNE'], correct: 2, explanation: 'Rewrite as $5 \\cdot \\frac{\\sin(5x)}{5x} \\to 5 \\cdot 1 = 5$' },
            { topic: '1.7', type: 'multiple_choice', stem: '$\\lim_{x \\to 4} \\frac{x - 4}{\\sqrt{x} - 2} =$', choices: ['$0$', '$2$', '$4$', 'DNE'], correct: 2, explanation: 'Since $x - 4 = (\\sqrt{x}-2)(\\sqrt{x}+2)$, we get $\\sqrt{x}+2 \\to 4$.' },
            ...
            { topic: '1.7', type: 'multiple_choice', stem: '$\\lim_{x \\to 1} \\frac{x^{1/3} - 1}{x - 1} =$', choices: ['$\\frac{1}{3}$', '$1$', '$3$', 'DNE'], correct: 0, explanation: 'Using $a^3 - b^3 = (a-b)(a^2+ab+b^2)$, we get $\\frac{1}{(x^{1/3})^2 + x^{1/3} + 1} \\to \\frac{1}{3}$.' },
            { topic: '4.7', type: 'multiple_choice', stem: '$\\lim_{x \\to 0} \\frac{1 - \\cos x}{x} =$', choices: ['$0$', '$1$', '$-1$', 'DNE'], correct: 0, explanation: 'L\'Hôpital\'s Rule: $\\frac{0}{0}$ form, so $\\frac{d(1-\\cos x)}{d(x)} = \\frac{\\sin x}{1} \\to 0$.' },
            ...
            { topic: '1.8', type: 'multiple_choice', stem: 'Why can\'t the Squeeze Theorem determine $\\lim_{x \\to 0} \\sin(1/x)$?', choices: ['The function is not defined at $x = 0$', 'The function is unbounded near $x = 0$', 'The bounding values $-1$ and $1$ are not equal', 'The Squeeze Theorem only works for polynomials'], correct: 2, explanation: 'The Squeeze Theorem requires both bounds to approach the same limit. Since $-1 \\neq 1$, we can only conclude $-1 \\leq \\lim \\leq 1$, not an exact value.' },
            ...
            { topic: '1.9', type: 'multiple_choice', stem: 'If algebra gives $\\lim_{x \\to 2} f(x) = 4$, what should you see in the $f(x)$ column as $x$ approaches 2?', choices: ['Values approaching 0', 'Values approaching 2', 'Values approaching 4', 'Values alternating between 2 and 4'], correct: 2, explanation: 'The $f(x)$ values should approach 4, confirming the algebraic result.' },
            ...
            { topic: '1.10', type: 'multiple_choice', stem: 'Which type of discontinuity cannot be made continuous by redefining the function at a single point?', choices: ['Removable', 'Hole in graph', 'Jump', 'Point discontinuity'], correct: 2, explanation: 'A jump discontinuity has different one-sided limits, so no single value can make it continuous.' },
            ...
            { topic: '2.9', type: 'multiple_choice', stem: '$\\frac{d}{dx}\\left(\\frac{1}{x^2}\\right)$ using quotient rule:', choices: ['$\\frac{-2}{x^2}$', '$\\frac{2}{x^3}$', '$\\frac{-1}{x^3}$', '$-2x^{-3}$'], correct: 3, explanation: '$\\frac{0 - 1(2x)}{x^4} = \\frac{-2x}{x^4} = -2x^{-3}$' },
            ...
            { topic: '2.10', type: 'multiple_choice', stem: '$\\frac{d}{dx}(\\tan x + \\sec x) =$', choices: ['$\\sec^2 x + \\sec x\\tan x$', '$\\sec^2 x - \\sec x\\tan x$', '$2\\sec^2 x$', '$\\tan^2 x + \\sec x$'], correct: 0, explanation: '$\\frac{d}{dx}(\\tan x) = \\sec^2 x$ and $\\frac{d}{dx}(\\sec x) = \\sec x \\tan x$.' },
            { topic: '2.10', type: 'multiple_choice', stem: 'Derive $\\frac{d}{dx}(\\tan x)$ using quotient rule on $\\frac{\\sin x}{\\cos x}$:', choices: ['$\\frac{\\cos^2 x - \\sin^2 x}{\\cos^2 x}$', '$\\frac{1}{\\sin^2 x}$', '$\\sec^2 x$', '$\\tan^2 x$'], correct: 2, explanation: '$\\frac{\\cos x \\cdot \\cos x - \\sin x \\cdot (-\\sin x)}{\\cos^2 x} = \\frac{\\cos^2 x + \\sin^2 x}{\\cos^2 x} = \\frac{1}{\\cos^2 x} = \\sec^2 x$.' },
            ...
            { topic: '3.2', type: 'multiple_choice', stem: 'If $e^y = x$, find $\\frac{dy}{dx}$:', choices: ['$e^y$', '$\\frac{1}{e^x}$', '$\\frac{1}{x}$', '$x$'], correct: 2, explanation: '$e^y \\frac{dy}{dx} = 1$, so $\\frac{dy}{dx} = \\frac{1}{e^y} = \\frac{1}{x}$.' },
            ...
            { topic: '3.2', type: 'multiple_choice', stem: 'If $y^2 = x$, then $\\frac{dy}{dx} =$', choices: ['$\\frac{1}{2y}$', '$2y$', '$\\frac{y}{2}$', '$\\frac{1}{\\sqrt{x}}$'], correct: 0, explanation: '$2y\\frac{dy}{dx} = 1 \\Rightarrow \\frac{dy}{dx} = \\frac{1}{2y}$' },
            ...
            { topic: '3.3', type: 'multiple_choice', stem: 'Using the inverse function derivative formula on $f(x) = \\ln x$ with $f^{-1}(x) = e^x$, find $\\frac{d}{dx}(e^x)$:', choices: ['$\\frac{1}{e^x}$', '$e^x$', '$x \\cdot e^{x-1}$', '$\\ln x$'], correct: 1, explanation: 'Since $\\frac{d}{dx}(\\ln x) = \\frac{1}{x}$, the inverse function formula gives $(f^{-1})\'(x) = \\frac{1}{f\'(f^{-1}(x))} = \\frac{1}{1/e^x} = e^x$.' },
            ...
            { topic: '4.3', type: 'multiple_choice', stem: 'Bacteria population $P(t) = 1000e^{0.5t}$. Growth rate at $t = 2$:', choices: ['$500e$', '$1000e$', '$250e$', '$500e^2$'], correct: 0, explanation: '$P\'(t) = 500e^{0.5t}$, $P\'(2) = 500e$' },
            ...
            { topic: '6.7', type: 'multiple_choice', stem: '$\\int_1^2 (3x^2 - 2x)\\, dx =$', choices: ['$4$', '$5$', '$3$', '$6$'], correct: 0, explanation: '$(x^3 - x^2)\\Big|_1^2 = (8-4) - (1-1) = 4$' },
